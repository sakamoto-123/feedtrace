...
public struct ConfettiCannon<T: Equatable>: View {
    ...
    // 添加 SoundManager 实例用于播放礼花音效，使用 @StateObject 确保实例持续存在
...
struct ConfettiAnimationView: View {
    ...
    var body: some View {
        shape
            ...
            .onAppear() {
                if firstAppear {
                    // 使用 withAnimation() 闭包启动无限重复的旋转动画
                    withAnimation(Animation.linear(duration: xSpeed).repeatForever(autoreverses: false)) {
                        move = true
                    }
                    firstAppear = false
                }
